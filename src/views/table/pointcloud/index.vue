<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="点云数据名称" min-width="150">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
    ...
    <form-point-cloud :dialog-form-visible.sync="dialogFormVisible" :form-data="formData" :local-form-data="localFormData" />
    <el-pagination
      background
...
<script>
import { getPointCloud, deletePointCloud } from '@/api/api-table-pointcloud'
import FormPointCloud from '@/views/form/components/form-pointcloud'

export default {
  components: { FormPointCloud },
  data() {
    return {
      list: null,
      listLoading: true,
  ...
  methods: {
    fetchData() {
      ...
      getPointCloud({ page: this.currentPage - 1, size: this.size }).then(response => {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        deletePointCloud({ index: row.index }).then(() => {
          this.fetchData()
          this.$message({
            type: 'success',
            message: '删除成功!'
    ...
    handleCurrentChange(page) {
      getPointCloud({ page: page - 1, size: this.size }).then(response => {
        this.list = response.data.content
      })
    },
    handleSizeChange(size) {
      if (this.currentPage === 1) {
        getPointCloud({ page: 0, size: size }).then(response => {
          this.list = response.data.content
        })
        return
      }
      if (this.currentPage * size <= this.total) {
        getPointCloud({ page: this.currentPage - 1, size: size }).then(response => {
          this.list = response.data.content
        })
      }
    }
